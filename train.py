...
def main(opt):
    # dataset
    train_dataset = NSDImageDataset(train_sub=opt.train_subs, val_sub=opt.val_subs, use_vc=True,
                image_norm=True, random_flip=False, phase='train')
    val_dataset = NSDImageDataset(train_sub=opt.train_subs, val_sub=opt.val_subs, use_vc=True,
                ...
                val_data_fraction=opt.val_data_fraction)
    train_dataloader = DataLoader(train_dataset, num_workers=4, batch_size=opt.batch_size, shuffle=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config_path", type=str, default='./models/finetune.yaml', 
